...
export class BudgetStore {
  ...
  async init() {
    ...
    liveQuery(() => db.budgetPeriods.orderBy("startDate").reverse().toArray()).subscribe(
      (periods) => {
  ...
  async loadActivePeriodData() {
    ...
    liveQuery(() =>
      db.budgetCategories.where("budgetPeriodId").equals(this.activePeriodId!).toArray(),
    ).subscribe((cats) => {
      this.categories = cats;
    });

    liveQuery(() =>
      db.expenses.where("budgetPeriodId").equals(this.activePeriodId!).toArray(),
    ).subscribe((exps) => {
      this.expenses = exps;
    });
  }
