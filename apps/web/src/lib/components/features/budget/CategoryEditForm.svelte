<script lang="ts">
  ...
  let allocatedAmount = $state(category.allocatedAmount);
...
<form onsubmit={(e) => { e.preventDefault(); handleSave(); }} class="space-y-4 pt-4">
  <div class="space-y-2">
  ...
  <div class="space-y-2">
    ...
    <div class="flex items-center gap-3">
      <Input id="cat-color" type="color" bind:value={color} class="w-12 h-10 p-1" />
      <span class="text-sm font-mono uppercase">{color}</span>
    </div>
  </div>
